# deep-research — Claude Code 運用ルール

任意テーマの高品質リサーチレポートを自動生成し、検証のうえ GitHub に公開する AI ディープリサーチエージェント。
...
4. **`session.json`** — 進捗状態。途中停止時は `/continue` で再開できるようにする
...
3. **報告前に URL 存在確認**: `gh api repos/KazuyaMurayama/deep-research/contents/PATH?ref=BRANCH`（または Contents API）でステータス200を確認。
...
### ファイル名
- 基本形 `<TOPIC>_YYYYMMDD.md`（**サフィックス・ハイフンなし**）。例: `RESEARCH_REPORT_20260603.md`
- 同日中の追加更新は `-v2`、`-v3`（例: `RESEARCH_REPORT_20260603-v2.md`）。
- 日付が変わったら v サフィックスをリセット（例: 翌日1回目 `RESEARCH_REPORT_20260604.md`）。
...
- `agents/01_research_architect.md` 〜 `04_quality_publisher.md` — 4段リサーチパイプライン。
- `outputs/` — 公開レポート。`session/` — 中間データ。`session.json` — 進捗状態。
- 各エージェント完了ごとに `session.json` を更新し、コミットする。前フェーズ出力が `null` のまま次へ進まない。
