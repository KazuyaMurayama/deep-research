...
> 左から右へ進み、右端まで行ったら下の段の左へ折り返します（印刷しやすいよう4列×2段）。

```mermaid
flowchart TB
    subgraph 上段[" "]
        direction LR
        S1["1. レート一覧で<br/>NASDAQ-100をタップ"] --> S2["2. 上部の<br/>「通常」タブを選択"] --> S3["3. 「買」を選択<br/>（新規買い建て）"] --> S4["4. 注文種類を選択<br/>成行/指値/逆指値/<br/>ストップリミット"]
    end
    subgraph 下段[" "]
        direction LR
