...
10. [前提の見直しと矛盾の処理（検証の透明性）](#10-前提の見直しと矛盾の処理検証の透明性)
11. [本書の限界・不確実性](#11-本書の限界不確実性)
12. [主要ソース一覧（Tier別・検証マーク付き）](#12-主要ソース一覧tier別検証マーク付き)
...
```mermaid
flowchart TB
    A["① SBI証券 総合口座<br/>（証券の入口・本人確認はここで完了）"] --> B["② 取引所CFD（くりっく株365）専用口座<br/>（追加で申込）"]
    B --> C["③ CFD口座へ資金を振替<br/>（総合口座→CFD口座）"]
    C --> D["④ 取引開始（NASDAQ-100を売買）"]
```
...
```mermaid
flowchart TB
    S1["1. レート一覧でNASDAQ-100をタップ"] --> S2["2. 上部の「通常」タブを選択"]
    S2 --> S3["3. 「買」を選択（新規買い建て）"]
    S3 --> S4["4. 注文種類を選択<br/>成行 / 指値 / 逆指値 / ストップリミット"]
    S4 --> S5["5. 数量（枚数）を入力"]
    S5 --> S6["6. 有効期限を選択（DAY/WEEK/GTC）"]
    S6 --> S7["7. 「確認する」で内容チェック"]
    S7 --> S8["8. 「注文する」で発注 → 完了画面"]
```
...
> ⚠️⚠️ **最悪、入金額以上を失います（元本超過損失）。** 例えば10万円を入れて3倍で1枚（約29万円相当）持ち、NASDAQが一晩で−15%動くと建玉の損失は約−4.4万円。さらに急落するとロスカットが間に合わず、**口座資金がマイナス（＝追加で支払う義務）**になることもあります。これがレバレッジ取引の怖さです。余裕資金で、低めの倍率から始めてください。
...
| 期間 | 取引時間 |
|---|---|
| **NY冬時間（通常期）** | 8:30 〜 翌6:00 |
| **NY夏時間（3月第2日曜〜11月第1日曜）** | 8:30 〜 翌5:00 |

- ほぼ24時間。休業は土日・米国市場休場日。日中終了〜翌開始の間（約2.5〜3.5時間）は取引不可 ✅
...
> 社内のマージン耐性検証では、**実効証拠金8%（≒レバレッジ12倍相当の余裕）を保てば過去52年で強制ロスカットはゼロ**でした。3倍運用はそれよりさらに安全側です（リポジトリ `MARGIN_CAPACITY_STRESS_RESULTS_20260617.md`）。
...
## 10. 前提の見直しと矛盾の処理（検証の透明性）

ユーザー依頼「前提から見直して」に対し、検証中に見つかった**社内資料の矛盾・前提の誤り**を隠さず記録します。

| 論点 | 旧前提（誤り/陳腐化） | 検証結果（本書の採用） | 根拠 |
|---|---|---|---|
| くりっく株365にNASDAQ商品はあるか | リポジトリ `G12` 文書：「NASDAQ製品は存在しない」「SBI CFD(店頭)で3%/年」 | **存在する**。2022-02-28上場「NASDAQ-100リセット付証拠金取引」 | [TFX上場告知](https://www.tfx.co.jp/newsfile/article/20211203-01) ✅ |
| 手数料は無料か | 「SBI CFDは手数料無料」 | くりっく株365のNASDAQ-100は**330円/枚（有料）**。無料は別商品（店頭CFD） | [SBIヘルプ](https://search.sbisec.co.jp/v2/popwin/help/cfd/27_attention_productsum.html) ✅ |
| 金利コストの大きさ | `product_costs.py`：SOFR+0.75% | 実勢の金利相当額 約4.5%/年（買いは支払い）。配当受取で一部相殺 | [くりっく株365公式](https://www.clickkabu365.jp/sp/contents16.html) ✅ |
| レバレッジの変え方 | （明示なし） | コース選択ではなく「建玉総額÷口座資金」で調整 | 検証（§6）✅ |

**矛盾の処理方針：** 現行正典 `CURRENT_BEST_STRATEGY.md`（2026-06-15）と `product_costs.py` が「くりっく株365 NASDAQ-100は存在する」前提に立っており、本Web検証もこれを支持しました。**旧 `G12` 文書の前提は陳腐化**しているため、本書は現行正典側を採用します。
