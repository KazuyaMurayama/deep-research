...
| 改善目標 | 第一推奨スキル | 補完スキル |
|---|---|---|
| ① レポートのビジュアル改善 | `Agents365-ai/mermaid-skill` | `chrisvoncsefalvay/claude-d3js-skill` |
...
| スキル | スター数 | 主な機能 | URL |
|---|---:|---|---|
| `WH-2099/mermaid-skill` | 約 56 | 23 種の Mermaid 図、`/mermaid` で呼び出し | https://github.com/WH-2099/mermaid-skill |
| `Agents365-ai/mermaid-skill` | 約 52 | 自然言語→Mermaid、構文検証ループ、PNG/SVG/PDF 出力 | https://github.com/Agents365-ai/mermaid-skill |
| `chrisvoncsefalvay/claude-d3js-skill` | 約 181 | D3.js でネットワーク図・ツリー図・対話的可視化 | https://github.com/chrisvoncsefalvay/claude-d3js-skill |

**使い分け：**
- `WH-2099/mermaid-skill`：23 種の網羅性を重視する場合
- `Agents365-ai/mermaid-skill`：自然言語生成と配布用の画像出力を重視する場合
- `claude-d3js-skill`：Mermaid で届かない大規模ネットワーク図・対話的可視化が必要な場合

> **推奨：** まず Mermaid 系を 1 つ基本導入し、表現力が必要になった段階で `claude-d3js-skill` を追加する。Mermaid は GitHub で標準表示されるため起点に最適。
...
|---|---|---|---|
| `code-review` プラグイン | anthropics/claude-code（公式） | 4 並列レビューエージェント＋確信度スコアリング（80 点以上のみ報告） | https://github.com/anthropics/claude-code |
...
| 3 | `obra/superpowers` | ②③ | 計画立案と検証の両方を備え、ワークフロー全体の骨格になる |
...
| 6 | `chrisvoncsefalvay/claude-d3js-skill` | ① | 約 181 スター。Mermaid で届かない高度・対話的可視化をカバー |
...
## 7. Claude for Small Business について（対象外の理由）
