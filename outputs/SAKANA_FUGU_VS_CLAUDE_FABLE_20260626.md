...
作成日: 2026-06-26
最終更新日: 2026-06-26（v3: §3戦略立案能力追加・スコア表13観点化）
...
> 4. 総合スコア（13観点）: **VS Code + Claude Code ＝ 復旧後 88.4 点 / 現状(Opus 4.8) 87.1 点** ＞ **Sakana Fugu Ultra ＝ 73.2 点**。Fugu が勝つのは**日本語品質・データ主権・規制耐性**の3点のみ。**投資戦略立案・バックテスト**においても Claude 優位（§3 参照）。
...
## 3. 戦略立案能力の比較 — 投資戦略フローで検証する

> **ユーザーの実況**: Opus 4.8 で約1万日の相場データからパターンを発見し高リターン策を生成済み。Fable で改善を重ねてきた。**Sakana Fugu + AI エージェントツールを組み合わせた場合、VS Code + Claude Code（Fable）に勝てるか？**

### 3-1. LLM の戦略立案能力 — 学術知見サマリ（2025〜2026）

| 知見 | 出典 | 戦略立案への含意 |
|---|---|---|
| マルチエージェントは単一エージェントより**エクイティポートフォリオ構築**で優位 | AlphaAgents (arXiv:2508.11152, 2025) | Fugu の多モデル協調はポートフォリオ最適化に有効な場合がある |
| ブル/ベア対立型マルチエージェント討論が**金融推奨の精度を向上** | arXiv:2411.04788 (2024) | 弱点発見・改善フェーズでのマルチエージェント討論は有効 |
| LLM の推論能力とマルチステップ計画の間には**構造的ギャップ**が存在 | "Why Reasoning Fails to Plan" (arXiv:2601.22311, 2026) | 長期バックテスト設計や複雑な多段階戦略には限界がある |
| RAG は基礎分析を改善するが**トレーディングシグナル推論を悪化**させる | FinTradeBench (arXiv:2603.19225, 2026) | データ注入の設計ミスが戦略精度を下げるリスクがある |
| 複雑な金融推論では**10〜20% のハルシネーションが残存** | arXiv:2503.22693 (2025) | 生成された数値・シグナル条件は必ず独立検証が必要 |
| **等トークン予算では単一エージェントがマルチエージェントに匹敵** | arXiv:2604.02460, Stanford (2026) | Fugu の優位はコスト余剰投入によるものでもある（コスト均等化で差は縮小） |
| AlphaForgeBench：投資戦略の**エンドツーエンド立案**専用ベンチマーク（Feb 2026） | AlphaForgeBench (2026) | Fugu / Fable の公式スコアは未公開 — ベンダー間比較は現時点では不可 |

### 3-2. バックテスト → 弱点発見 → 改善ループ：モデル別能力比較

ユーザーのワークフローを5フェーズに分解して比較する。

| フェーズ | Opus 4.8（今すぐ） | Fable 5（復旧後） | Sakana Fugu Ultra |
|---|---|---|---|
| **① 戦略仮説生成**（パターン認識・仮説構造化） | HLE 57.9%・GDPval-AA 1,890 Elo。1万日データの構造化・パターン言語化に強い | Opus より高い文脈維持・複雑仮説生成力（SWE-Bench Pro 80.3%が示す構造分析力） | ブル/ベア討論でアイデアの反証を自動実施。多視点仮説に有利 |
| **② バックテスト実装**（Python・pandas/vectorbt/zipline） | SWE-Bench Pro 69.2%。Claude Code IDE 統合でライブデバッグ可能 | SWE-Bench Pro 80.3% で最高クラス。エラーの根本原因特定まで一貫 | SWE-Bench Pro 73.7%。多モデルクロスチェックで誤り修正するが速度が犠牲 |
| **③ 弱点特定**（過学習検出・ウォークフォワード・体制変化検出） | ⭕ 深い論理推論でデータの構造的弱点を特定。ただし計算実行は外部ツール任せ | ⭕ Opus より高い文脈で複雑な相互作用を把握 | ⭕ 多モデル視点で見落とし低減。ただしハルシネーション10〜20%に要注意 |
| **④ 改善案立案**（ハイパーパラメータ調整・フィルタ追加・リスク制御） | サブエージェント並列起動で複数案を同時評価（Claude Code Skills活用） | ⭕ Fable のエージェント基盤でより高精度な多案評価 | ブル/ベア討論型で改善案の脆弱性を事前検証。ただし従量課金が急増 |
| **⑤ 反復速度**（改良サイクルの速さ） | ⭐ **最速**。IDE 統合・低レイテンシ・コスト安定で高頻度反復に向く | ⭐ 復旧後は最速かつ最高精度 | △ 1プロンプト最大34分。高品質だが反復頻度が大幅に制限される |

### 3-3. 戦略立案特化スコア（8観点・各100点）

| # | 戦略立案観点 | Fable 5（復旧後） | Opus 4.8（現状） | Fugu Ultra |
|---|---|---:|---:|---:|
| S1 | 仮説生成・パターン言語化力 | **92** | 88 | 85 |
| S2 | バックテストコード生成（Python実装力） | **90** | 78 | 80 |
| S3 | 弱点特定（過学習・regime 検出） | **88** | 85 | 82 |
| S4 | 多視点対立討論（ブル/ベア） | 78 | 75 | **88** |
| S5 | 改善案の多様性・反証能力 | **84** | 80 | 85 |
| S6 | 反復速度（改良サイクル） | **88** | **87** | 45 |
| S7 | ハルシネーション抑止・数値精度 | **85** | 82 | 72 |
| S8 | データ注入・RAG設計の適切さ | **80** | 78 | 75 |
| | **合計（/800）** | **685** | **653** | **612** |
| | **100点換算** | **85.6** | **81.6** | **76.5** |

> AlphaForgeBench 等の公式スコアはベンダー未公開のため、上記は公開ベンチマーク（SWE-Bench Pro・HLE・GDPval-AA 等）と学術知見の複合推定値。

### 3-4. 戦略立案における マルチエージェント協調の得失
...
**Fugu が有利な場面（単発・深い討論）:**
- 弱点発見フェーズのブル/ベア対立討論（arXiv:2411.04788）で「自分が見落とした前提」を自動抽出
- 多様な仮説生成：GPT（定量）・Claude（論理構造）・Gemini（長文分析）の役割分担
- 最終案クロスチェック：「この条件は1万日のどの体制で有効か」のような多角的検証

**Fable / Opus が有利な場面（高頻度反復）:**
- バックテスト→修正→再バックテストのような高頻度ループ（Fugu の34分待ちは致命的ボトルネック）
- コードの正確さが要求される場面（SWE-Bench Pro でClaude 優位・ハルシネーション10〜20%残存リスクも考慮）
- 等トークン予算での比較（arXiv:2604.02460：同一コスト前提では単一エージェントがマルチに匹敵）

**実践的推奨（本ユーザーのワークフロー）:**

| タスク | 推奨手段 | 追加コスト |
|---|---|---|
| バックテスト実装・高頻度反復改善 | **Opus 4.8 ＋ Claude Code（今すぐ）** | +0 円 |
| 弱点発見・多視点討論 | **Fugu Ultra（単発・Fable代替として）** | $5〜30/回程度（従量課金） |
| 戦略コード品質向上 | **Fable（復旧後）** | +0 円（Max枠内） |
...
| 13 | 投資戦略立案・バックテスト | **90** | 82 | 76 | SWE-Bench Pro + HLE + 反復速度の複合。Fugu は討論強いが34分待ちで反復不向き（§3詳細） |
| | **合計（/1300）** | **1149** | **1132** | **952** | |
| | **100点換算（平均）** | **88.4** | **87.1** | **73.2** | |

> **注**: スコアはベンダー発表ベンチマーク・実機検証（KEITO 氏）・公式機能一覧の複合評価。用途によって重み付けは変わる。

### スコアの読み解き

- **A-1（Fable復旧後）が勝つ**: 実開発力・戦略立案コード品質・反復速度で B を明確にリード。Fable 5 復旧後の最良ケース。
- **A-2（Opus 4.8現状）が勝つ**: 安定性・規制リスクでは Fable より高い。今すぐ使える現実解。戦略立案でも「高頻度反復」と「コスト効率」でFuguを大きく上回る。
- **B（Fugu Ultra）が勝つ**: **日本語品質（97点）・データ主権（90点）・規制耐性（85点）の3点のみ**。「手直し不要の美しい日本語ビジネス文書」の量産では圧倒的。戦略立案でも多視点討論（S4: 88点）は有利だが、34分待ちと青天井課金がボトルネック。
...
> **「Fable 並みの開発力・戦略立案力」は、すでに払っている Max $100/月の中に Opus 4.8 として眠っている。Sakana Fugu は財布を溶かす日本語名人——開発・投資戦略の反復改善には使わず、弱点の多視点討論と文章の仕上げ職人として単発で呼べ。**
...
**投資戦略立案・金融LLM（学術論文）**
- [AlphaAgents: Multi-Agent for Equity Portfolio Construction（arXiv:2508.11152）](https://arxiv.org/abs/2508.11152)
- [Multi-Agent Bull/Bear Debate for Financial Recommendations（arXiv:2411.04788）](https://arxiv.org/abs/2411.04788)
- [Why Reasoning Fails to Plan（arXiv:2601.22311）](https://arxiv.org/abs/2601.22311)
- [FinTradeBench: RAG hurts trading signal reasoning（arXiv:2603.19225）](https://arxiv.org/abs/2603.19225)
- [Hallucination in Financial LLMs（arXiv:2503.22693）](https://arxiv.org/abs/2503.22693)
- [Single-Agent vs Multi-Agent under Equal Token Budget（arXiv:2604.02460, Stanford 2026）](https://arxiv.org/abs/2604.02460)
- [AlphaForgeBench: End-to-End Trading Strategy Formulation（2026）](https://arxiv.org/search/?query=AlphaForgeBench&searchtype=all)
