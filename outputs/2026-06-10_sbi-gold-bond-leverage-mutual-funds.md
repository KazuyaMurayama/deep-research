...
> **改訂メモ（同日改訂）**: 初版レビューにより (1) **Tracersゴールドプラス系2本を追加**、(2) TMF経費率0.95→**0.90%**・NASDAQ100 3倍ブル信託報酬を**1.524%**に修正、(3) 円短期金利0.5→**0.75%**（日銀2025年12月利上げ反映）、(4) 米ドルプラスコースの仕組みを正確化。**第2改訂**: 過去調査レポート（TQQQ・QLD・TMF・UBT・UGL・2036・1540×信用・SBI金CFD）のデータを序盤の結論表に統合し、資産クラス別（NASDAQ／ゴールド／ボンド／複合）にグループ整理。種別（米国ETF／国内ETN／投信／CFD）列を追加。

---

## ■ 結論（先に全体像）

### 結論①：コスト一覧表（全商品横断比較・年率・SOFR 3.63%／円金利 0.75%前提）

> コストは**4種類**に分解（①信託報酬＝明示／②金利コスト＝先物価格に内包・**目論見書の信託報酬には含まれない**・米ドル資産は≈SOFR／③為替ヘッジコスト＝円ヘッジ型のみ・日米金利差／④トレード（先物ロール）コスト＝内包・変動）。「レバレッジ」欄は **総エクスポージャー** を示す。「概算トータル」はボラドラッグ（日次リバランス型のみ）を**別途表記**。

| 商品名 | 種別 | 対象 | レバレッジ | ①信託報酬/年(税込) | ②金利コスト/年【内包・推定】 | ③為替ヘッジコスト | ④トレード/ロール | 購入時(SBI) | **概算トータル/年【推定】**（ボラドラッグ別途） | SBI |
|---|---|---|---|---|---|---|---|---|---|---|
| **── NASDAQグループ ──** | | | | | | | | | | |
| **TQQQ（ProShares UltraPro QQQ）** | 米国ETF | NASDAQ100 | **3倍** | 0.86% | ≈SOFR×2 ＝ **7.3%** | 無（USD建） | ~0.1% | **0円** | **≈ 8.3%** ＋ドラッグ≈14.5% | ✅ |
| **QLD（ProShares Ultra QQQ）** | 米国ETF | NASDAQ100 | **2倍** | 0.95% | ≈SOFR×1 ＝ **3.6%** | 無 | ~0.1% | **0円** | **≈ 4.7%** ＋ドラッグ≈4.8% | ✅ |
| **NASDAQ100 3倍ブル（大和AM）** | 投信 | NASDAQ100 | **3倍** | 1.524% | ≈SOFR×2 ＝ **7.3%** | ≈日米金利差 **約3%** | ~0.3% | **0円** | **≈ 12%** | ✅ |
| **── ゴールドグループ ──** | | | | | | | | | | |
| **Tracers NASDAQ100ゴールドプラス** | 投信 | NASDAQ100＋**金** | 2倍（株1x＋金1x） | 0.2189% | ≈SOFR×1 ＝ **3.6%**＋金リース | 無 | ~0.2% | **0円** | **≈ 4.3%** | ✅ |
| **Tracers S&P500ゴールドプラス** | 投信 | S&P500＋**金** | 2倍（株1x＋金1x） | 0.1991% | ≈SOFR×1 ＝ **3.6%**＋金リース | 無 | ~0.2% | **0円** | **≈ 4.3%** | ✅ |
| **1540（純金ETF）× 信用買い** | 国内ETF×信用 | 金（円建） | **≈2倍** | 0.44%＋信用金利2.80% | なし（信用コスト込み） | 円建 | 小 | 売買手数料 | **≈ 3.24%**（最安・ドラッグ無し） | ✅ |
| **2036（NEXT NOTES 金先物ダブル・ブルETN）** | 国内ETN | 金（円建） | **2倍** | ETN管理費0.80% | ≈円金利×1 ＝ **0.75%** | 円建 | ~0.3% | 売買手数料 | **≈ 1.9%** ＋ドラッグ≈2.6% | ✅ |
| **UGL（ProShares Ultra Gold）** | 米国ETF | 金 | **2倍** | 0.95% | ≈SOFR×1 ＝ **3.6%** | 無 | ~0.1% | **0円** | **≈ 4.7%** ＋ドラッグ≈2.6% | ✅ |
| **ジャパン・ゴールドファンド（ブル2倍型）** | 投信 | 金（円建） | **2倍** | 2.14% | ≈円金利×1 ＝ **0.75%** | 円建 | ~0.3% | 要確認 | **≈ 3.2%** | ⚠️ 要確認 |
| **SBI 金CFD（3倍設定運用）** | CFD | 金 | 最大20倍 | スプレッドのみ | 金利＋スプレッド内包 | 無 | 無 | **0円** | **≈ 14%**【推定・3倍時、ドラッグ無し】 | ✅ |
| **── ボンドグループ ──** | | | | | | | | | | |
| **auAM米国債スーパーロング・ブル3倍（円コース）** | 投信 | 米国超長期国債 | **3倍** | 0.4334% | ≈SOFR×2 ＝ **7.3%** | ≈日米金利差 **約3%** | ~0.2% | **0円** | **≈ 10〜11%** | ✅ |
| **auAM米国債スーパーロング・ブル3倍（米ドルプラスコース）** | 投信 | 米国超長期国債＋ドル | **3倍** | 0.4334% | ≈SOFR×2 ＝ **7.3%** | 0%（ドルロング戦略） | ~0.2% | **0円** | **≈ 7.5〜8%** ＋為替 | ✅ |
| **TMF（Direxion 20+年米国債3倍ブル）** | 米国ETF | 米国超長期国債 | **3倍** | 0.90% | ≈SOFR×2 ＝ **7.3%** | 無 | ~0.1% | **0円** | **≈ 8.3%** ＋ドラッグ≈4.7% | ✅ |
| **UBT（ProShares Ultra 20+ Year Treasury）** | 米国ETF | 米国超長期国債 | **2倍** | 0.95% | ≈SOFR×1 ＝ **3.6%** | 無 | ~0.1% | **0円** | **≈ 4.7%** ＋ドラッグ≈1.6% | ⚠️ 要確認 |
| **── 複合グループ ──** | | | | | | | | | | |
| **米国3倍4資産リスク分散ファンド** | 投信 | 米国4資産（金含む） | **3倍** | 0.4675% | ≈SOFR×2 ＝ **7.3%** | 一部ヘッジ（変動） | ~0.2% | **0円** | **≈ 8〜9%** | ✅ |
| **── 日本株（参考）──** | | | | | | | | | | |
| **SBI日本株4.3ブル** | 投信 | 日本株 | **4.3倍** | 〜0.968% | ≈円金利×3.3 ＝ **2.5%** | 円建 | ~0.3% | **0円** | **≈ 3.8%** | ✅ |
