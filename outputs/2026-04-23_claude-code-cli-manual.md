...
**要点**：
- **インストール作業** → PowerShell（Claude Code とは別ウィンドウ）
- **Git操作** → Git Bash
- **Claude Code 起動後** → Claude Code 内部ターミナル（Bashツール）にほぼ丸投げ
- **コマンドプロンプト（CMD）は原則不要**（パス区切り・Unix命令の非互換が多い）

> ⚠️ **Claude Code の `>` 入力欄はClaudeへの指示を書く場所**です。PowerShell や Git Bash のコマンドを貼り付けると「Pasted text」として扱われ、実行されません。winget・npm・git などは必ず別のターミナルウィンドウで実行してください。
...
ブラウザが開かない場合は、表示されるURLを手動でコピーしてブラウザに貼り付けてください。

#### ② Anthropicアカウントでログイン

- claude.ai と同じアカウントでOK
- **Pro / Max サブスクリプション**または**API Key**が必要
- ログイン成功するとターミナルに戻る
...
`> ` が表示されたら準備完了。ここに自然言語で指示を入力します。

#### ④ CLAUDE.md があれば自動読み込み

プロジェクトに `CLAUDE.md` があると起動時に自動で参照します。ない場合は何も起きません（エラーではない）。

> ⚠️ **よくある誤解**：`claude` を実行しても何も起きないように見える場合、ブラウザが裏で開いている可能性があります。タスクバーを確認してください。
...
`> ` はプロンプト記号（入力欄の先頭）です。実際には `>` の後ろの文字だけを入力してください。
...
### Step 1：gh CLI をインストール（書き込み操作が必要な場合のみ）

公開リポジトリの**読み取り**（コード確認・構造把握など）は gh CLI なしで動作します。
**PR作成・Issue操作・git push など書き込み系**を使うときだけインストールが必要です。

> ⚠️ **重要**：以下のコマンドは Claude Code の `>` 欄ではなく、**別途開いた PowerShell ウィンドウ**で実行してください。Claude Code の入力欄に貼り付けると「Pasted text」として認識されてしまいます。

```powershell
# 【別のPowerShellウィンドウで実行】Claude Codeは閉じなくてOK
...
> 💡 **まず読み取りから試して、PR作成が必要になったらインストール**という順番でOKです。
...
#### 方法1：`/usage` でコスト・消費を確認（メイン）

```
> /usage
```

セッションのコスト・プラン使用量・アクティビティ統計をまとめて表示します。**コスト管理の主力コマンド**。

#### 方法2：`/context` でコンテキスト残量を可視化

```
> /context
```

残りコンテキストをグリッド表示し、最適化の提案も出します。

#### 方法3：StatusLine でリアルタイム表示

**StatusLine** は Claude Code の状態をターミナル下部に常時表示する機能。`context_window.remaining_percentage` を含むJSONを毎ターン受け取れます。

**最も簡単な方法**：Claude Code 内から `/statusline` コマンドで対話設定できます。

```
> /statusline
```

外部パッケージ不要でインタラクティブに設定完了します。

より高機能なカスタマイズには **ccstatusline**（コミュニティ製）が便利です：
...
# -p がプロンプト指定の標準フラグ。--bare を加えると hooks/MCP/CLAUDE.md をスキップして高速動作
claude -p "READMEを最新化" > result.md
claude --bare -p "READMEを最新化" > result.md   # 高速版（hooks無効）
```

```powershell
# 【PowerShell で実行】
claude -p "READMEを最新化" | Out-File result.md
```

```yaml
# GitHub Actions（Linux環境）
- name: Auto-update README
  run: claude -p "READMEを最新化" > result.md
```

これで**Claude をシェルの一部品として**扱えます。CI失敗時の自動修正、定期メンテナンスタスクなどに最適。

### 5. /loop：定期実行（bundled skill）
...
> 💡 `/loop` `/simplify` `/team-onboarding` は**bundled skill**（CLIに同梱されたプロンプト集）です。純粋なハードコードコマンドではなく、`.claude/skills/` に置いたカスタムスキルと同じ仕組みで動きます。内容を確認・上書きすることも可能です。

### 6. /simplify：並列コードレビュー（bundled skill）
...
### 7. /team-onboarding：チーム展開（bundled skill）
...
| パターン | 症状 | 対策 |
|---------|------|------|
| **広域探索** | 「調査して」と言うと100ファイル読んでコンテキスト枯渇 | 「〇〇ファイルだけ見て」と範囲を明示 |
| **確認なし実装** | テストなしで一気に実装→後でバグだらけ | `/plan` で設計確認→段階的に実装 |
| **1セッション多タスク** | 別タスクを追加し続けてコンテキスト混濁 | タスク完了ごとに `/clear` して新規セッション |
| **出力を鵜呑み** | それらしいコードを確認せずにマージ | 必ず動作確認・テスト実行後にコミット |
