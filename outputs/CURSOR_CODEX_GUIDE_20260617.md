...
作成日: 2026-06-17
最終更新日: 2026-06-17（v3：Cursor+Codex vs VS Code+Claude Code の100点満点スコア比較を §1 に新設）

> 高校生でもそのままなぞれば動かせる「最短手順」と「所要時間」をまとめたガイドです。
> 公式ドキュメント＋実機解説記事を相互照合のうえ手順を組み立て、引っかかりやすいポイントは末尾の「検証と修正メモ」に反映済みです。
>
> **v2 → v3 での主な改訂**：
> **v2**：① Node.js 必須バージョンを v22 以上に修正、② `npm i -g codex`（スコープなし）の罠を警告に追加、③ Agent モードの権限境界を明文化、④ 「既存 GitHub リポジトリを Codex に引き継いで検証する方法」セクション追加（ローカル拡張機能／Codex Cloud／PR レビュー連携の3方式比較）。
> **v3**：⑤ 新 §1「Cursor + Codex」と「VS Code + Claude Code」の **100点満点・10観点スコア比較**を追加（総合：A 82.9 / B 85.7）、用語整理を §2 へ移動。
...
## 1. 代表的な2つの活用スタイル：100点満点スコア比較

「Cursor で Codex を使う」道と並んで、もう一つ代表的な選択肢が **「VS Code で Claude Code 拡張機能を使う」道**です。本ガイドは前者の手順書ですが、選択を間違えないよう **10 観点で100点満点比較** します（2026 年 6 月時点・公開一次情報ベース）。

### 1.1 比較対象

| | A. **Cursor + OpenAI Codex 拡張機能** | B. **VS Code + Claude Code 拡張機能** |
|---|---|---|
| ベースエディタ | Cursor（VS Code 派生・AI ネイティブ） | VS Code（Microsoft 標準） |
| AI エンジン | OpenAI Codex（GPT-5.x Codex 系） | Anthropic Claude（Opus 4.8 / Sonnet 4.6 / Haiku 4.5 / Fable 5） |
| 形態 | IDE 拡張機能＋Cursor 標準 AI を併用可、Codex Cloud にクラウド委譲可 | VS Code 拡張機能が CLI（Claude Code）をラップ、Web/iPhone 版もあり |
| 必要なプラン | ChatGPT Plus/Pro 以上 or OpenAI API キー | Claude Pro/Max 以上 or Anthropic API キー |

### 1.2 スコア表（10 観点 × 100 点満点）

| # | 観点 | A. Cursor + Codex | B. VS Code + Claude Code | 評価コメント |
|---|---|---:|---:|---|
| 1 | **AI エージェント的な使い方**（自律実行・委譲） | **88** | **95** | B は Subagents + Agent Teams + Hooks で多層エージェントを標準装備。A も Codex Cloud で真の自律実行が可能だが、エージェント階層化の機能は B に厚みあり |
| 2 | **スキルの活用やワークフロー**（Skills / Slash Commands / Hooks 等） | **78** | **95** | B は Skills（呼び出し可能な手順）／Hooks（自動実行）／Plugins／MCP／Slash Commands の役割分担が明確で体系化済み。A は AGENTS.md と Cloud の Setup Script が中心で機能数では劣る |
| 3 | **使いやすさ・理解しやすさ**（初学者の参入容易度） | **90** | **78** | A は GUI 主体で「インストール→ログイン→チャット」の最短ルートが直感的。B は CLI 思想が前提で、CLAUDE.md／Skills／Hooks の設計概念を理解する初期コストがかかる |
| 4 | **UI の点**（操作の見えやすさ・整理） | **85** | **80** | A は Cursor が AI ネイティブ IDE として洗練、サイドパネルの一体感が高い。B はターミナル＋差分ペインの機能美で、慣れると効率的だが視覚的わかりやすさでは A 優位 |
| 5 | **コンテキスト圧縮などの管理方法** | **72** | **92** | B は 200K を**実効**で安定使用、Opus 系で 1M トークン beta（MRCR v2 で 76%）、自動 compaction＋`/compact` ポリシー＋subagent による文脈隔離が確立。A は Cursor 側で実効 70〜120K に切り詰められる報告あり、Codex Cloud で環境隔離は可能だが圧縮制御は弱い |
| 6 | **タスクの並行実行** | **92** | **85** | A は Codex Cloud がブラウザ／iPhone から **真の並列バックグラウンド**処理に対応（複数 PR を同時待ち）。B は Agent Teams で局所並列＋Web 版で cloud session も可だが、A の「投げて忘れる」体験が一歩リード |
| 7 | **モデル選択の自由度** | **80** | **62** | A は Cursor 本体側で GPT-5.3-Codex／Claude／Gemini／Composer 等を切替可、Codex 拡張機能内では OpenAI 系が中心。B は Anthropic モデル専一（その代わりモデルファミリーの一貫性は高い） |
| 8 | **エコシステム拡張性**（MCP / Plugins / SDK 等） | **80** | **96** | B は Hooks／MCP／Skills／Plugins／Agent SDK が公式に揃い、コミュニティ拡張が爆発的（gstack / gbrain 等）。A は MCP 対応＋SDK あり、Subagents も登場したが拡張の総量と多様性は B が上 |
| 9 | **コスト構造の透明性・効率** | **78** | **86** | 両者プラン制で透明。独立ベンチでは Claude Code が同じタスクを **33K トークン**で完了したのに対し Cursor が **188K トークン**消費したとの報告あり（5.5x の効率差）。B のトークン効率がコスト面で優位 |
| 10 | **学習リソース・コミュニティ** | **86** | **88** | A は OpenAI 周辺の日本語記事が早期に大量蓄積。B は CLAUDE.md／Skills 文化が 2026 年に急速に成熟、企業導入事例も豊富 |

### 1.3 総合スコア（単純平均）

| 評価 | A. Cursor + Codex | B. VS Code + Claude Code |
|---|---:|---:|
| **総合（10 観点平均）** | **82.9 / 100** | **85.7 / 100** |
| 強みカテゴリ | 並行実行・GUI 直感性・モデル自由度 | エコシステム・コンテキスト・スキル体系 |
| 弱みカテゴリ | コンテキスト圧縮・スキル体系 | 初学者の理解負荷・モデル選択 |

> 📌 **総合では B（VS Code + Claude Code）が僅差で上**ですが、**「すぐ動かして触りたい」「並列バックグラウンドで PR を量産したい」なら A が上回ります**。

### 1.4 どちらを選ぶ？（30 秒判断フロー）

```
あなたの優先事項は？
├─ 「とにかく最短で動かしたい・GUI 重視」      → A. Cursor + Codex
├─ 「並列タスクをクラウドに投げて待つ」         → A. Cursor + Codex（Codex Cloud）
├─ 「複数モデルを切替えて使い比べたい」         → A. Cursor + Codex
├─ 「自分のワークフローを Hooks/Skills で作り込む」 → B. VS Code + Claude Code
├─ 「リポジトリ全体や長大ログを扱う」           → B. VS Code + Claude Code
├─ 「トークン効率・コスト最小化を重視」         → B. VS Code + Claude Code
└─ 「迷う」                                    → **両方併用が 2026 年の主流**（多くの上級エンジニアは両方を使い分けている）
```

> 💡 **本ガイドの本論は A の手順**ですが、B にも興味がある方は同リポジトリの [Claude Code CLI 初心者向け完全マニュアル](https://github.com/KazuyaMurayama/deep-research/blob/main/outputs/2026-04-23_claude-code-cli-manual.md) を参照してください。

### 1.5 評価の前提と限界

- **時点**：2026 年 6 月時点の公開情報（公式ドキュメント・独立ベンチ・主要解説記事）に基づく
- **採点根拠**：公式機能の有無、独立ベンチマーク（MRCR v2、トークン効率）、複数比較記事の評価傾向を統合
- **採点者**：本レポート著者（男座員也）。公平を期して両者の弱点も明示
- **誤差**：UI 評価など主観が含まれる項目は ±5 点の幅で見ること
- **更新頻度**：両者とも月次で機能が変わるため、3 か月以上経過した本表は再評価推奨
...
## 4. 手順 A：IDE 拡張機能で Codex を使う（**最も簡単・推奨**）
...
6. インストールが終わると、サイドバーに **Codex のアイコン**が追加される（見当たらないときは §8 トラブルシュート参照）
...
## 5. 手順 B：Cursor のターミナルから Codex CLI を使う（自由度重視）
...
## 6. 既存 GitHub リポジトリを Codex に引き継いで検証を進める方法
...
### 6-1. 方式 ①：ローカル clone → Cursor の Codex 拡張機能で検証する
...
### 6-2. 方式 ②：Codex Cloud（chatgpt.com/codex）に GitHub を接続する
...
4. **AGENTS.md がリポにあれば自動参照される**
   - § 6-1 のテンプレで作っておけば、Cloud 側でもそのまま効く
...
### 6-3. 方式 ③：既存 PR で `@codex review` / `@codex fix` を使う
...
6. **アクティビティバーの注意を 3 箇所重複から 1 箇所（§8 トラブルシュート）に集約**
...
**Next Action（推奨）**：
0. **§1 のスコア比較を 1 分で確認**して、Cursor + Codex（本ガイド）で進むか、VS Code + Claude Code に切替えるかを判断
1. Cursor + Codex で進むなら、**§4 手順 A の A-1〜A-4** を 5 分で実施（拡張機能インストール → ChatGPT サインイン → 「このプロジェクトの構成をざっと説明して」で動作確認）
2. 動作確認できたら **§6-1 方式 ①** で、自分の既存 GitHub リポジトリをクローンして AGENTS.md を1枚追加 → Codex に「全体把握 → テスト実行 → 修正案」を依頼
3. 慣れたら **§6-2 方式 ②（Codex Cloud）** を試して、PR が自動で返ってくる体験をする
